...
<template>
  <div v-if="step === AssetStep.DETAILS" ref="colors">
      ...
      <TransitionGroup name="expand-200">
        <div v-for="(item, i) of model" :key="i" class="flex flex-col items-center gap-2">
          ...
          <UButtonGroup class="form-input-group w-full">
            <InputFloating v-model="item.color" type="text" class="w-full" placeholder="Color" required />
            <UPopover>
            ...
            <UButton icon="lucide:trash-2" variant="outline" color="error" class="px-3" :ui="{ base: 'rounded-lg' }" @click="model.splice(i, 1)" />
          </UButtonGroup>
    ...
    <UButton icon="lucide:plus" size="lg" variant="subtle" class="w-full justify-center rounded-lg mt-3" @click="model.push({ name: '', description: '', color: '#FFFFFF' })" />
  </div>
